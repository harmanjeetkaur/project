...
?>
		<?php $handle = fopen("counter.txt", "r"); if(!$handle){ echo "could not open the file" ; } else { $counter = ( int ) fread ($handle,20) ; fclose ($handle) ; $counter++ ; echo" <strong> You Are Visitor Number:". $counter . " </strong > " ; $handle = fopen("counter.txt", "w" ) ; fwrite($handle,$counter) ; fclose ($handle) ; } ?>
...
$con=mysqli_connect("localhost", "root", "", "php_beginner_crud_level_1");

// Check connection
if (mysqli_connect_errno()) {
  echo "Failed to connect to MySQL: " . mysqli_connect_error();
}
$result=mysqli_query($con,"select * from products where `name` like '%$key%'"); 
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    echo "<tr>";
        ...
        echo "<td>";
            // read one record 
            echo "<a href='uploadss.php?id={$id}' class='btn btn-info m-r-1em'>Read</a>";
